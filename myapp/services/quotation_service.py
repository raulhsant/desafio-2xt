...
class QuotationService(object):
    QuotationDTO = namedtuple('QuotationDTO',
                              ['name', 'baggage_damage_coverage', 'medic_expenses_coverage', 'max_age', 'min_age',
    ...
    @classmethod
    def map_to_dto(cls, quotation: dict, product_list: list):
        ...
        return cls.QuotationDTO(name=quotation.get('product_name'), net_price=prices_tuple[0],
                                elder_net_price=prices_tuple[1],
                                baggage_damage_coverage=cls.get_coverage_value(quotation, (20,)),
                                medic_expenses_coverage=cls.get_coverage_value(quotation, (1, 2)),
                                elder_age=desired_product.get('elder_age'),
                                max_age=desired_product.get('max_age'),
                                min_age=desired_product.get('min_age'))

    @classmethod
    def get_coverage_value(cls, quotation, coverage_ids: tuple):
        coverage_wrapper = [coverage for coverage in quotation.get('coverages') if coverage.get('coverage_id') in coverage_ids]
        if coverage_wrapper:
